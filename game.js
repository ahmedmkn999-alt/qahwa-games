...
// ===== FIREBASE (inline - no module needed) =====
// Firebase يتحمل من CDN عادي مش module
window.DB = null; // هيتعبي بعد ما Firebase يتحمل
...
function initFirebase() {
  try {
    ...
    window.DB = {
      async createRoom(code, data)           { await db.ref(`rooms/${code}`).set(data); },
      async getRoom(code)                    { const s = await db.ref(`rooms/${code}`).once('value'); return s.exists() ? s.val() : null; },
      async updateRoom(code, updates)        { await db.ref(`rooms/${code}`).update(updates); },
      async updatePlayer(code, pid, updates) { await db.ref(`rooms/${code}/players/${pid}`).update(updates); },
      listenRoom(code, cb)                   { const r = db.ref(`rooms/${code}`); r.on('value', s => cb(s.exists() ? s.val() : null)); return () => r.off(); },
      async deleteRoom(code)                 { await db.ref(`rooms/${code}`).remove(); },
      async addPlayerToRoom(code, pid, data) { await db.ref(`rooms/${code}/players/${pid}`).set(data); },
      async removePlayer(code, pid)          { await db.ref(`rooms/${code}/players/${pid}`).remove(); }
    };
    console.log('✅ Firebase ready');
  } catch(e) {
    console.log('Firebase error (offline mode ok):', e);
  }
}
...
let audioCtx = null;
let masterGain = null; // كل الأصوات والموسيقى بتعدي من هنا
...
function toggleSound() {
  soundOn = !soundOn;
  // كتم أو تشغيل كل الأصوات عبر masterGain
  if (masterGain) {
...
function playSound(type) {
  ...
  try {
    ...
    osc.connect(gain);
    gain.connect(masterGain); // بيعدي من masterGain

    const sounds = {
      tap:     { freq: 520,  type: 'sine',     dur: 0.06, vol: 0.08 },
      success: { freq: 640,  type: 'sine',     dur: 0.18, vol: 0.12 },
      flip:    { freq: 380,  type: 'sine',     dur: 0.12, vol: 0.08 },
      tick:    { freq: 700,  type: 'sine',     dur: 0.04, vol: 0.05 },
      alert:   { freq: 280,  type: 'sine',     dur: 0.3,  vol: 0.12 },
      win:     { freq: 523,  type: 'sine',     dur: 0.4,  vol: 0.12 },
    };
    ...
    if (type === 'win') {
      ...
      setTimeout(() => playTone(784, 0.35), 260);
    }
  } catch(e) {}
}
...
// ===== AMBIENT MUSIC =====
// موسيقى عربية ambient هادية باستخدام مقام راست
let musicOn = false;
let musicNodes = [];
let musicInterval = null;

const RAST_NOTES = [264, 297, 330, 352, 396, 440, 462, 528]; // مقام راست تقريبي
...
function startMusic() {
  ...
  try {
    ...
    drone(132, 0.025);   // دو عميق
    drone(264, 0.02);    // دو
    drone(396, 0.015);   // صول

    // نغمات عشوائية من مقام راست كل فترة
    let noteIdx = 0;
    musicInterval = setInterval(() => {
      if (!musicOn) return;
      try {
        const freq = RAST_NOTES[Math.floor(Math.random() * RAST_NOTES.length)];
        const osc  = ctx.createOscillator();
        const gain = ctx.createGain();
        const rev  = ctx.createBiquadFilter();
        rev.type = 'lowpass';
        rev.frequency.value = 1200;
        osc.connect(rev);
        rev.connect(gain);
        gain.connect(masterGain);
        osc.type = 'sine';
        osc.frequency.value = freq;
        gain.gain.setValueAtTime(0, ctx.currentTime);
        gain.gain.linearRampToValueAtTime(0.035, ctx.currentTime + 0.3);
...
// ===== CATEGORY CARDS DATA =====
const categoryMeta = {
  'أماكن':      { gradient: 'linear-gradient(135deg,#6C63FF,#3A35CC)', svg: '<circle cx="22" cy="18" r="8" fill="rgba(255,255,255,0.3)"/><path d="M22 26 L22 38" stroke="white" stroke-width="2.5" stroke-linecap="round"/><rect x="8" y="28" width="28" height="18" rx="4" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><rect x="17" y="34" width="10" height="12" rx="2" fill="white" opacity="0.4"/>' },
  'أكل':        { gradient: 'linear-gradient(135deg,#FF9500,#CC6600)', svg: '<circle cx="22" cy="20" r="12" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><path d="M16 20 Q22 14 28 20 Q22 26 16 20Z" fill="rgba(255,255,255,0.5)"/><line x1="22" y1="32" x2="22" y2="40" stroke="white" stroke-width="2.5" stroke-linecap="round"/><line x1="14" y1="36" x2="30" y2="36" stroke="white" stroke-width="2" stroke-linecap="round"/>' },
  'أفلام':      { gradient: 'linear-gradient(135deg,#FF4E6A,#CC1A35)', svg: '<rect x="6" y="12" width="32" height="22" rx="4" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><polygon points="18,18 18,28 28,23" fill="white" opacity="0.7"/><line x1="6" y1="8" x2="38" y2="8" stroke="white" stroke-width="2" stroke-linecap="round"/><line x1="10" y1="6" x2="10" y2="10" stroke="white" stroke-width="2"/><line x1="18" y1="6" x2="18" y2="10" stroke="white" stroke-width="2"/><line x1="26" y1="6" x2="26" y2="10" stroke="white" stroke-width="2"/><line x1="34" y1="6" x2="34" y2="10" stroke="white" stroke-width="2"/>' },
  'مسلسلات':   { gradient: 'linear-gradient(135deg,#9B59B6,#6C3483)', svg: '<rect x="4" y="10" width="36" height="24" rx="3" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><rect x="14" y="34" width="16" height="4" rx="2" fill="rgba(255,255,255,0.3)"/><line x1="8" y1="37" x2="36" y2="37" stroke="white" stroke-width="2"/><circle cx="22" cy="22" r="7" fill="none" stroke="white" stroke-width="1.5"/><polygon points="19,19 19,25 26,22" fill="white" opacity="0.7"/>' },
  'كرتون':      { gradient: 'linear-gradient(135deg,#00C896,#007A5E)', svg: '<circle cx="22" cy="18" r="10" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><circle cx="17" cy="16" r="3" fill="white" opacity="0.6"/><circle cx="27" cy="16" r="3" fill="white" opacity="0.6"/><path d="M16 22 Q22 27 28 22" stroke="white" stroke-width="2" fill="none" stroke-linecap="round"/><path d="M12 8 Q16 4 18 8" stroke="white" stroke-width="1.5" fill="none"/><path d="M26 8 Q28 4 32 8" stroke="white" stroke-width="1.5" fill="none"/>' },
  'حيوانات':   { gradient: 'linear-gradient(135deg,#E67E22,#A04000)', svg: '<ellipse cx="22" cy="24" rx="13" ry="10" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><circle cx="22" cy="14" r="7" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><circle cx="18" cy="12" r="2" fill="white" opacity="0.6"/><circle cx="26" cy="12" r="2" fill="white" opacity="0.6"/><ellipse cx="15" cy="8" rx="3" ry="4" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1"/><ellipse cx="29" cy="8" rx="3" ry="4" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1"/>' },
  'مهن':        { gradient: 'linear-gradient(135deg,#3498DB,#1A5276)', svg: '<rect x="8" y="18" width="28" height="20" rx="4" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><rect x="14" y="12" width="16" height="8" rx="3" fill="rgba(255,255,255,0.15)" stroke="white" stroke-width="1.5"/><line x1="8" y1="26" x2="36" y2="26" stroke="white" stroke-width="1.5"/><rect x="18" y="26" width="8" height="6" rx="2" fill="white" opacity="0.4"/>' },
  'بلاد':       { gradient: 'linear-gradient(135deg,#1ABC9C,#0E6655)', svg: '<circle cx="22" cy="22" r="14" fill="rgba(255,255,255,0.15)" stroke="white" stroke-width="1.5"/><ellipse cx="22" cy="22" rx="6" ry="14" fill="none" stroke="white" stroke-width="1" opacity="0.5"/><line x1="8" y1="22" x2="36" y2="22" stroke="white" stroke-width="1" opacity="0.5"/><line x1="8" y1="16" x2="36" y2="16" stroke="white" stroke-width="0.8" opacity="0.3"/><line x1="8" y1="28" x2="36" y2="28" stroke="white" stroke-width="0.8" opacity="0.3"/>' },
  'مشاهير':    { gradient: 'linear-gradient(135deg,#F39C12,#7D6608)', svg: '<polygon points="22,6 26,16 38,16 28,23 32,34 22,27 12,34 16,23 6,16 18,16" fill="rgba(255,255,255,0.25)" stroke="white" stroke-width="1.5"/>' },
  'برندات':     { gradient: 'linear-gradient(135deg,#E74C3C,#7B241C)', svg: '<path d="M10 30 L22 10 L34 30 Z" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><circle cx="22" cy="24" r="6" fill="rgba(255,255,255,0.3)"/>' },
  'منتخبات':   { gradient: 'linear-gradient(135deg,#27AE60,#145A32)', svg: '<circle cx="22" cy="22" r="14" fill="rgba(255,255,255,0.15)" stroke="white" stroke-width="1.5"/><path d="M8 22 Q15 15 22 22 Q29 29 36 22" stroke="white" stroke-width="1.5" fill="none"/><path d="M8 22 Q15 29 22 22 Q29 15 36 22" stroke="white" stroke-width="1" fill="none" opacity="0.4"/>' },
  'هدوم':       { gradient: 'linear-gradient(135deg,#8E44AD,#4A235A)', svg: '<path d="M14 10 L10 18 L16 18 L16 36 L28 36 L28 18 L34 18 L30 10 L26 14 L22 10 L18 14 Z" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5" stroke-linejoin="round"/>' },
  'لاعبو كرة': { gradient: 'linear-gradient(135deg,#2ECC71,#186A3B)', svg: '<circle cx="22" cy="22" r="13" fill="rgba(255,255,255,0.15)" stroke="white" stroke-width="1.5"/><path d="M22 9 L25 18 L35 18 L27 24 L30 34 L22 28 L14 34 L17 24 L9 18 L19 18 Z" fill="rgba(255,255,255,0.3)"/>' },
  'أغاني':      { gradient: 'linear-gradient(135deg,#E91E8C,#880E4F)', svg: '<circle cx="28" cy="12" r="6" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><circle cx="28" cy="30" r="6" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/><line x1="22" y1="12" x2="14" y2="20" stroke="white" stroke-width="2" stroke-linecap="round"/><line x1="22" y1="30" x2="14" y2="20" stroke="white" stroke-width="2" stroke-linecap="round"/><circle cx="14" cy="28" r="5" fill="rgba(255,255,255,0.2)" stroke="white" stroke-width="1.5"/>' },
};
...
function buildCategoryCards() {
  ...
  Object.keys(categoryItems).forEach((cat, i) => {
    const meta  = categoryMeta[cat] || { gradient:'linear-gradient(135deg,#6C63FF,#3A35CC)', svg:'' };
    const count = categoryItems[cat].length;
    const card  = document.createElement('div');
    card.className = 'cat-card';
    card.style.cssText = `background:${meta.gradient};box-shadow:0 6px 20px rgba(0,0,0,0.3);animation-delay:${i*0.04}s`;
    card.innerHTML = `
      <div class="cat-card-illustration">
        <svg viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">${meta.svg}</svg>
      </div>
      <div class="cat-card-info">
        <div class="cat-card-title">${cat}</div>
        <div class="cat-card-count">${count} عنصر</div>
      </div>
...
function startLocalGame(cat) {
  ...
  // Shuffle question order (مين يسأل مين)
...
function showRevealCard() {
  ...
  document.getElementById('reveal-next-btn').textContent =
    gameState.revealIndex < gameState.players.length - 1 ? 'التالي ➡️' : 'ابدأ اللعبة 🎮';
...
function startGameTimer() {
  ...
  // ترتيب الأسئلة - مين يسأل مين
  const orderList = document.getElementById('question-order-list');
  orderList.innerHTML = '';
  const order = gameState.questionOrder;
  order.forEach((name, i) => {
    ...
    d.innerHTML = `
      ...
      <div style="color:var(--gold);font-weight:700">${next}</div>
    `;
...
function resolveVoting() {
  ...
  if (accused && accused.role === 'spy') {
    gameState.votedOutSpyIdx = mostVoted;
    showSpyGuess(); // الجاسوس يخمن
  } else {
    // صوتوا غلط - الجاسوس فاز
...
function confirmGuess() {
  ...
  if (selectedGuess === gameState.location) {
    // الجاسوس خمّن صح
    showResults('spy_win_guess');
  } else {
    // الجاسوس خمّن غلط - المدنيون يفوزوا
...
/*
  ...
  المدنيون صوتوا صح + الجاسوس خمّن صح   → من اختاره +100 بس
...
function showResults(outcome) {
  ...
  if (outcome === 'civilians_win') {
    // المدنيون صوتوا صح والجاسوس خمّن غلط
    bannerClass = 'result-banner win';
    emoji = '🎉'; title = 'المدنيون فازوا!'; subtitle = 'الجاسوس اتكشف وخمّن غلط';
    ...
    emoji = '🕵️'; title = 'الجاسوس فاز!'; subtitle = 'المدنيون صوتوا على الغلط!';
    gameState.players.forEach(p => { if (p.role === 'spy') p.score += 200; });
    playSound('alert');

  } else if (outcome === 'spy_win_guess') {
    // الجاسوس خمّن صح - اللي صوت عليه ياخد 100 بس
    bannerClass = 'result-banner lose';
    emoji = '🎯'; title = 'الجاسوس خمّن وفاز!'; subtitle = 'اللي صوت عليه ياخد 100 نقطة';
    // اللي صوت على الجاسوس ياخد 100
    if (spyIdx >= 0) {
      gameState.players.forEach(p => {
        if (p.role === 'civilian' && p.vote === spyIdx) p.score += 100;
      });
    }
    gameState.players.forEach(p => { if (p.role === 'spy') p.score += 200; });
    playSound('alert');
...
function spawnConfetti() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    c.style.cssText = `left:${Math.random()*100}%;background:${colors[Math.floor(Math.random()*colors.length)]};animation-duration:${Math.random()*2+1.5}s;animation-delay:${Math.random()}s;`;
    wrap.appendChild(c);
  }
}
...
function startLocalGame() {
  ...
  if (selectedCategories.length === 0) { toast('اختار تصنيف واحد على الأقل ⚠️'); return; }
  playSound('success');
